...
class FLVDecoder extends EventEmitter {
  ...
  logTag(tag) {
    ...
    if (tag.type === FLV.SCRIPT) { //script
      console.log(`[${tag.timestamp}] script frame`);
    } else if (tag.type === FLV.AUDIO) { //audio
      console.log(`[${tag.timestamp}] audio frame ${audioFormatName}`);
    } else if (tag.type === FLV.VIDEO) { //video
  ...
  readTag(data) {
    ...
    if (tag.type === FLV.SCRIPT) { //script
      ...
      if (tag.info.audioFormat == FLV.AUDIO_AAC) {  // aac
